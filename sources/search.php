<?php 
    ...
    $limit = ' limit '.$startpoint.','.$per_page;
  	...
  	$sql = "select *,tenkhongdau_$lang as tenkhongdau from #_baiviet where type in ('san-pham') and hienthi=1 $where $order_by ".$limit;
  	
    $tintuc = $db->rawQuery($sql);

    $sqlq = "select COUNT(*) as `numb` from #_baiviet where type in('san-pham')	and hienthi=1 $where $order_by";
