<?php  if(!defined('_source')) die("Error");
    ...
    if(empty($id) && empty($idl) && empty($idc) && empty($idi) && empty($ids)){
        ...
        $tintuc=$db->rawQuery("select id,tenkhongdau_$lang as tenkhongdau,ten_$lang,mota_$lang,masp,photo, rank ,thumb,giaban,giacu,ngaytao,luotxem,link from #_baiviet where hienthi=1 and type=? $order_by limit $startpoint,$per_page",array($type));
