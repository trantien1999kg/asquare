<?php  if(!defined('_source')) die("Error");
    ...
    if(!empty($show)){
        ...
        if($com=='san-pham' || $com=='thuong-hieu'){ 
    ...
    if(!empty($idl)){
        ...
        if($row_list){
            ...
            $sqlc = "select *,tenkhongdau_$lang as tenkhongdau , ten_$lang as ten, from #_baiviet where hienthi=1 and id_list=? and type=? $order_by limit $startpoint,$per_page";
            $tintuc=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau , ten_$lang as ten from #_baiviet where hienthi=1 and id_list=? and type=? $order_by limit $startpoint,$per_page",array($row_list['id'],$type));
            // var_dump($db->getLastError());
           
            $tintuc1=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau , ten_$lang as ten from #_baiviet_list where hienthi=1 and type=?",array('san-pham'));
    ...
    if(!empty($idc)){
        ...
        if($row_cat){
            ...
            $tintuc=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau , ten_$lang as ten from #_baiviet where hienthi=1 $combo and id_cat=? and type=? $order_by limit $startpoint,$per_page",array($row_cat['id'],$type));
    ...
    if(!empty($idi)){
        ...
        if($row_item){
            ...
            $tintuc=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau , ten_$lang as ten from #_baiviet where hienthi=1 $combo and id_item=? and type=? $order_by limit $startpoint,$per_page",array($row_item['id'],$type));
    ...
    if(!empty($ids)){
        ...
        if($row_sub){
            ...
            $tintuc=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau , ten_$lang as ten from #_baiviet where hienthi=1 $combo and id_sub=? and type=? $order_by limit $startpoint,$per_page",array($row_sub['id'],$type));
    ...
    if(empty($id) && empty($idl) && empty($idc) && empty($idi) && empty($ids)){
        ...
        $tintuc=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau , tacgia ,ten_$lang as ten from #_baiviet where hienthi=1 $combo and type=? $order_by limit $startpoint,$per_page",array($type));
        ...
        $row_list=$db->rawQuery("select id,ten_$lang, tenkhongdau_$lang as tenkhongdau, ten_$lang as ten ,type from #_baiviet_list where hienthi=1 and type=? order by stt asc,id desc",array($type));
        ...
        $tintuc=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau , ten_$lang as ten  from table_baiviet where hienthi=1 $combo and type=?{$subWhere} order by stt asc, id desc limit 0,10",array($type));
